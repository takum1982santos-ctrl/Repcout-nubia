  ...
  useEffect(()=>{
    ...
    const init=async()=>{
      try{
        ...
        let bu="cpu";try{await loadScript(WASM_URL);window.tf.wasm.setWasmPaths("https://cdn.jsdelivr.net/npm/@tensorflow/tfjs-backend-wasm@4.11.0/dist/");await window.tf.setBackend("wasm");await window.tf.ready();bu="wasm";}catch(e){try{await window.tf.setBackend("cpu");await window.tf.ready();}catch(e2){}}
...
function PoseView({color,exerciseId,onRep,active,facingMode,onFlipCamera,onReady,onPhaseChange,burpeeFlexTarget=1}){
  ...
  return(<div style={{position:"relative"}}><canvas ref={canvasRef} width={280} height={210} style={{width:"100%",borderRadius:"16px",display:"block",border:`2px solid ${repFlash?color:color+"33"}`,boxShadow:repFlash?`0 0 20px ${color}55`:"none",transition:"border-color 0.1s,box-shadow 0.1s"}}/>{onFlipCamera&&<button onClick={onFlipCamera} style={{position:"absolute",top:"8px",right:"8px",background:"rgba(0,0,0,0.6)",border:`1px solid ${color}44`,borderRadius:"8px",color:"#fff",fontSize:"16px",padding:"4px 8px",cursor:"pointer",lineHeight:1}}>{facingMode==="user"?"📷":"🤳"}</button>}<div style={{position:"absolute",bottom:"8px",left:"50%",transform:"translateX(-50%)",display:"flex",alignItems:"center",gap:"8px",background:"rgba(0,0,0,0.8)",border:`1px solid ${color}44`,borderRadius:"20px",padding:"4px 12px",whiteSpace:"nowrap"}}>{liveAngle!==null&&<span style={{fontSize:"13px",fontFamily:"monospace",color:pColor,fontWeight:"bold"}}>{liveAngle}°</span>}<span style={{fontSize:"9px",letterSpacing:"2px",color:pColor}}>{status==="ACTIVO"?pLabel:status}</span>{status==="ACTIVO"&&<div style={{width:"36px",height:"5px",borderRadius:"3px",background:"rgba(255,255,255,0.1)",overflow:"hidden"}}><div style={{height:"100%",borderRadius:"3px",width:`${liveConf*100}%`,background:liveConf>0.6?"#00C9A7":liveConf>0.35?"#FFD700":"#FF4D4D",transition:"width 0.1s"}}/></div>}</div>{liveDbg&&<div style={{position:"absolute",top:"8px",left:"8px",background:"rgba(0,0,0,0.8)",color:"#0f0",fontSize:"9px",fontFamily:"monospace",padding:"4px 6px",borderRadius:"4px",maxWidth:"55%",wordBreak:"break-all",whiteSpace:"pre-line",lineHeight:"1.3"}}>{liveDbg}</div>}</div>);
}
    ...
    if(detail.mode==="giga"){
      return(<div style={{width:"100%",maxWidth:"420px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
          {(detail.byExercise||[]).map((item,i)=>{const ex2=exercises.find(e=>e.id===item.exerciseId),C2=ex2?.color||"#00C9A7";return(<div key={i} style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${C2}33`,borderRadius:"12px",padding:"12px 16px",display:"flex",alignItems:"center",gap:"12px"}}><span style={{fontSize:"20px"}}>{ex2?.icon}</span><div style={{flex:1,textAlign:"left"}}><div style={{fontSize:"14px",letterSpacing:"2px",color:C2}}>{ex2?.name?.toUpperCase()}</div>{item.exerciseId==="burpee_sin_salto"&&detail.burpeeFlexTotal>0&&<div style={{fontFamily:"sans-serif",fontSize:"10px",color:"#888",marginTop:"2px"}}>{detail.burpeeFlexTotal} flexiones en la plancha</div>}</div><div style={{textAlign:"right"}}><div style={{fontSize:"22px",color:C2,lineHeight:1}}>{item.reps}</div><div style={{fontSize:"8px",color:"#555",letterSpacing:"2px"}}>REPS</div></div></div>);})}        </div>
      </div>);
    }
...
function RepCountApp(){
  ...
  const[gigaRepsPerSet,setGigaRepsPerSet]=useState(1);
  const[gigaTotalSets,setGigaTotalSets]=useState(1);
  const[gigaRestDuration,setGigaRestDuration]=useState(60);
  const[gigaMode,setGigaMode]=useState("series");
  const[gigaSeriesMode,setGigaSeriesMode]=useState("reps");
  const[gigaSetDuration,setGigaSetDuration]=useState(60);
  const[gigaCurrentSet,setGigaCurrentSet]=useState(1);
  const[gigaSetRoundIdx,setGigaSetRoundIdx]=useState(0);
  const[gigaRestLeft,setGigaRestLeft]=useState(0);
    ...
    <div style={{minHeight:"100vh",background:"#0A0A0F",fontFamily:"'Bebas Neue','Arial Black',sans-serif",color:"#fff",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px",position:"relative",overflow:"hidden"}}>
        ...
        {gigaSeries.map(g=>(
          <div key={g.id} onClick={()=>selectGigaRecipe(g)} style={{padding:"16px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"14px",marginBottom:"10px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}}>
            ...
            <div style={{display:"flex",alignItems:"center"}}>
              <button onClick={(e)=>{e.stopPropagation();setGigaEditId(g.id);setGigaEditName(g.name);setGigaEditSteps(g.steps);setScreen("gigaseries_editor");}} style={{background:"none",border:"1px solid rgba(255,255,255,0.1)",color:"#555",cursor:"pointer",fontSize:"18px",padding:"6px 10px",borderRadius:"8px",lineHeight:1,marginRight:"6px"}}>✏️</button>
              <button onClick={(e)=>{e.stopPropagation();const updated=gigaSeries.filter(x=>x.id!==g.id);saveGigaSeries(updated);setGigaSeries(updated);}} style={{background:"none",border:"none",color:"#f44336",cursor:"pointer",fontSize:"16px",padding:"4px"}}>🗑</button>
            </div>
          </div>
        ))}
      </div>}
      ...
      {screen==="prog_rest"&&activeSession&&(()=>{
        ...
        return(<div style={{width:"100%",maxWidth:"420px",zIndex:1,textAlign:"center"}}>
          ...
          <div style={{position:"relative",width:"180px",height:"180px",margin:"0 auto 20px"}}>
            <svg width="180" height="180" style={{transform:"rotate(-90deg)"}}><circle cx="90" cy="90" r="78" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="6"/><circle cx="90" cy="90" r="78" fill="none" stroke={bt.color} strokeWidth="6" strokeLinecap="round" strokeDasharray={`${2*Math.PI*78}`} strokeDashoffset={`${2*Math.PI*78*(progRestLeft/(progIsRestWithin?block?.restWithin||15:block?.restAfter||90))}`} style={{transition:"stroke-dashoffset 1s linear",filter:`drop-shadow(0 0 8px ${C})`}}/></svg>
      ...
      {screen==="gigaseries_counting"&&gigaActiveRecipe&&(()=>{
        ...
        return(<div style={{width:"100%",maxWidth:"420px",zIndex:1,textAlign:"center"}}>
          ...
          {poseActive&&exData&&<div style={{marginBottom:"10px"}}><PoseView key={`giga-${gigaRoundIdx}-${gigaStepIdx}-${cameraKey}`} color={exColor} exerciseId={exData.id} onRep={gigaSimulateRep} active={poseActive} facingMode={facingMode} onFlipCamera={()=>{setFacingMode(m=>m==="user"?"environment":"user");setCameraKey(k=>k+1);}} onReady={()=>setPoseReady(true)} onPhaseChange={()=>{}} burpeeFlexTarget={step?.burpeeFlex>0?step.burpeeFlex:1}/></div>}          <div style={{display:"flex",gap:"8px",marginBottom:"8px"}}>
      ...
      return(<div style={{width:"100%",maxWidth:"420px",zIndex:1,textAlign:"center"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"24px"}}>
            {Object.entries(totals).map(([exId,total])=>{const ex=exercises.find(e=>e.id===exId),C2=ex?.color||"#00C9A7";return(<div key={exId} style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${C2}33`,borderRadius:"12px",padding:"12px 16px",display:"flex",alignItems:"center",gap:"12px"}}><span style={{fontSize:"20px"}}>{ex?.icon}</span><div style={{flex:1,textAlign:"left"}}><div style={{fontSize:"14px",letterSpacing:"2px",color:C2}}>{ex?.name?.toUpperCase()}</div>{exId==="burpee_sin_salto"&&burpeeFlexTotal>0&&<div style={{fontFamily:"sans-serif",fontSize:"10px",color:"#888",marginTop:"2px"}}>{burpeeFlexTotal} flexiones en la plancha</div>}</div><div style={{textAlign:"right"}}><div style={{fontSize:"28px",color:C2,lineHeight:1}}>{total}</div><div style={{fontSize:"8px",color:"#555",letterSpacing:"2px"}}>REPS</div></div></div>);})}          </div>
